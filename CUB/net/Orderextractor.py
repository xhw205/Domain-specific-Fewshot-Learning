...
class OEmbeddingNetwork(nn.Module):  #OrderNet
    def __init__(self):
        super(OEmbeddingNetwork, self).__init__()
        ...
        self.fc1 = nn.Linear(512*2, 2)
        self.fc2 = nn.Linear(512*2, 100)
    ...
    def forward(self,x):
        ...
        a = layer4
        b1 = self.conv11(layer4)
        b2 = self.conv12(layer4)
        b = b1 * b2
        x = torch.cat([a, b], 1)
        out = x
        x= self.avgpool(out)
