...
def mini_imagenet_folders():
    train_folder = '/home/ws/datasets/phytoplankton/fewshot/SmallTest'
    test_folder = '/home/ws/datasets/phytoplankton/fewshot/SmallTest'
...
class FewShotDataset(Dataset):
    ...
    def __init__(self, task, split='train', transform=None,transformatt=None ,target_transform=None):
        self.transform = transform # Torch operations on the input image
        self.target_transform = target_transform
        self.transformatt = transformatt
        self.task = task
        self.split = split
...
class MiniImagenet(FewShotDataset):
    ...
    def __getitem__(self, idx):
        ...
        raw_root = '/home/ws/datasets/phytoplankton/fewshot/SmallTestAtt/'
        image_root_att = os.path.join(raw_root, *(image_root.split('/')[-2:]))
        image_att = Image.open(image_root_att).convert('RGB')
        if self.transform is not None:
            image = self.transform(image)
            image_att = self.transformatt(image_att)
        label = self.labels[idx]
        return image, label, image_att
...
def get_mini_imagenet_data_loader(task, num_per_class=1, split='train',shuffle = False):
    ...
    if split == 'train':
        ...
        transform = transforms.Compose([transforms.Resize((224,224)), transforms.ToTensor(), normalize])
    t_transform = transforms.Compose([transforms.Resize((84,84)), transforms.ToTensor(), normalize])

    dataset = MiniImagenet(task,split=split, transform=transform, transformatt=t_transform)
    if split == 'train':
